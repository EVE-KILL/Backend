...
export default {
    ...
    run: ({ args }) => {
        createWorker('importCharacter', async (job: Job) => {
            let character = job.data.character;
            ...
            console.log('Character Update:', job?.id, '( CharacterID:', job.data.character.character_id, `) | ${err.message} | ${process.env.ESI_URL || 'https://esi.evetech.net/'}latest/characters/${job?.data.characterId}/`);
        }).on('completed', (job: Job) => {
            console.log('Character Update:', job.id, '( CharacterID:', job.data.character.character_id, ') | Completed');
        }).on('ready', () => {
            console.log('Character Worker Ready');
        });

        createWorker('importCorporation', async (job: Job) => {
            let corporation = job.data.corporation;
            ...
            console.log('Corporation Update:', job?.id, '( CorporationID:', job.data.corporation.corporation_id, `) | ${err.message} | ${process.env.ESI_URL || 'https://esi.evetech.net/'}latest/corporations/${job?.data.corporationId}/`);
        }).on('completed', (job: Job) => {
            console.log('Corporation Update:', job.id, '( CorporationID:', job.data.corporation.corporation_id, ') | Completed');
        }).on('ready', () => {
            console.log('Corporation Worker Ready');
        });

        createWorker('importAlliance', async (job: Job) => {
            let alliance = job.data.alliance;
            ...
            console.log('Alliance Update:', job?.id, '( AllianceID:', job.data.alliance.alliance_id, `) | ${err.message} | ${process.env.ESI_URL || 'https://esi.evetech.net/'}latest/alliances/${job?.data.allianceId}/`);
        }).on('completed', (job: Job) => {
            console.log('Alliance Update:', job.id, '( AllianceID:', job.data.alliance.alliance_id, ') | Completed');
        }).on('ready', () => {
