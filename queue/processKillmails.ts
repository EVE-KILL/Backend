...
export default {
    ...
    run: async ({ args }) => {
        ...
        createWorker('killmail', async (job: Job) => {
            ...
            cliLogger.error(`Killmail Parser: ${job?.id} ( KillID: ${job?.data.killmailId} ) | ${err.message} | ${(process.env.ESI_URL || 'https://esi.evetech.net/')}latest/killmails/${job?.data.killmailId}/${job?.data.killmailHash}/`);
