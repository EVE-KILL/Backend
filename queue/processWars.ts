    ...
    run: ({ args }) => {
        ...
        createWorker('war', async (job: Job) => {
            ...
            if (warData.aggressor.ships_killed > 0 || warData.defender.ships_killed > 0) {
                cliLogger.info(`War Update: ${job.id} ( WarID: ${job.data.warId} ) | Processing Killmails`);
            ...
            cliLogger.error(`War Update: ${job?.id} ( WarID: ${job?.data.warId} ) | ${err.message} | ${(process.env.ESI_URL || 'https://esi.evetech.net/')}latest/wars/${job?.data.warId}/`);
