...
export default defineEventHandler(async (event) => {
    const shipId: number | null = event.context.params?.id ? parseInt(event.context.params.id) : null;
    if (!shipId) {
    ...
    const fittingResults = await Promise.all(killmails.map(async killmail => {
        ...
        const fittingHash = crypto.createHash('md5').update(stableStringify(filteredFitting)).digest('hex');
        return {
    ...
    const finalFittings = sortedFittings.slice(0, limit).map((fit: any, index: number) => {
        ...
        fit.svg = generateSvg(`https://images.evetech.net/types/${shipId}/render`, fit.fitting_cost, fit.ship_value, index + 1);
        return fit;
    });
...
async function generateFittingCost(fitting: any): Promise<number> {
    ...
    for (const slot in fitting) {
        for (const item of fitting[slot]) {
            const price = await Prices.findOne({ type_id: item.type_id, region_id: 10000002 }).sort({ date: -1 });
            if (price?.average) {
...
async function generateFitting(items: IItem[]) {
    ...
    (Object.keys(fitting) as Array<keyof typeof fitting>).forEach(slot => {
...
function generateSvg(shipImageUrl: string, fitCost: number, shipCost: number, rank: number): string {
