async function calculateShortStats(type: string, id: number, days: number) {
    ...
    // Kills
    let attackerKillmails = Killmails.find(
        days > 0
            ? {
                  ["attackers." + type]: id,
                  kill_time: {
    ...
    // Losses
    let victimKillmails = Killmails.find(
        days > 0
            ? {
                  ["victim." + type]: id,
                  kill_time: {
    ...
    for await (let killmail of victimKillmails) {
...
async function calculateFullStats(type: string, id: number, days: number): Promise<IFullStats> {
    ...
    let heatMap = {};
    for (let i = 0; i < 24; i++) {
        let hourString = "h" + i.toString().padStart(2, "0");
    ...
    for await (const killmail of attackerKillmails) {
        ...
        // Heat map (UTC hour)
        const hourKey =
            "h" + killTime.getUTCHours().toString().padStart(2, "0");
        stats.heatMap[hourKey]++;

        // Blob factor
        if (
            Array.isArray(killmail.attackers) &&
            killmail.attackers.length > 10
        ) {
            blobKills++;
        }
        ...
        if (Array.isArray(killmail.attackers)) {
            for (const attacker of killmail.attackers) {
                if (attacker[type] === id) {
                    ...
                    stats.mostUsedShips[entityShipId] = stats.mostUsedShips[
                        entityShipId
                    ] || {
                        count: 0,
                        name: shipName,
                ...
                if (corpId && corpId !== id) {
                    stats.fliesWithCorporations[corpId] = stats
                        .fliesWithCorporations[corpId] || {
                        count: 0,
                        name: corpName,
                    };
                ...
                if (alliId && alliId !== id) {
                    stats.fliesWithAlliances[alliId] = stats.fliesWithAlliances[
                        alliId
                    ] || {
                        count: 0,
                        name: alliName,
    ...
    for await (const lossmail of victimKillmails) {
        ...
        if (type === "character_id") {
            ...
            const cynoCutoff =
                new Date("2019-09-01T00:00:00Z").getTime() / 1000;
        ...
        if (Array.isArray(lossmail.attackers)) {
            for (const attacker of lossmail.attackers) {
                ...
                if (corpId) {
                    stats.diesToCorporations[corpId] = stats.diesToCorporations[
                        corpId
                    ] || {
                        count: 0,
                        name: corpName,
                ...
                if (alliId) {
                    stats.diesToAlliances[alliId] = stats.diesToAlliances[
                        alliId
                    ] || {
                        count: 0,
                        name: alliName,
    ...
    const sortTop10 = <T extends { count: number }>(obj: Record<number, T>) => {
        ...
        return entries.slice(0, 10).reduce((acc, [key, val]) => {
    ...
    const sortTop10withKillmails = (
        obj: Record<
            number,
            { count: number; name: string; killmails: number[] }
        >
    ) => {
        const entries = Object.entries(obj).sort(
            (a, b) => b[1].count - a[1].count
    ...
    stats.fliesWithCorporations = sortTop10withKillmails(
        stats.fliesWithCorporations
    );
