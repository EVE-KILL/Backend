...
export default {
    ...
    run: async ({ args }) => {
        ...
        let folders = fs.readdirSync(tmpDir, { withFileTypes: true }).filter(dirent => dirent.isDirectory()).map(dirent => dirent.name);
            ...
            for (const file of files) {
                ...
                if (fs.existsSync(folder)) {
                    ...
                    for (const killmailFile of killmails) {
                        ...
                        let killmail = new KillmailsESI(km);
                        try {
                            await killmail.save();
                        } catch (err) {
