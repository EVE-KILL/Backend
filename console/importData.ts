...
export default {
    ...
    run: async ({ args }) => {
        ...
        let characterQueue = createQueue('importCharacter');
        for await (const { value: character } of pipeline) {
            ...
            await characterQueue.add('importCharacter', { character: characterData }, {
                priority: 1,
                attempts: 10,
                backoff: {
                    type: 'fixed',
        ...
        let allianceQueue = createQueue('importAlliance');
        for (let alliance of alliances) {
            ...
            await allianceQueue.add('importAlliance', { alliance: allianceData }, {
                priority: 1,
                attempts: 10,
                backoff: {
                    type: 'fixed',
        ...
        let corporationQueue = createQueue('importCorporation');
        for (let corporation of corporations) {
            ...
            await corporationQueue.add('importCorporation', { corporation: corporationData }, {
                priority: 1,
                attempts: 10,
                backoff: {
                    type: 'fixed',
